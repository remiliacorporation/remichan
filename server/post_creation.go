...
func trackThreadEngagement(w http.ResponseWriter, r *http.Request) {
	address, jwt := util.ConnectedWalletAddress(r)
	ip, err := auth.GetIP(r)
	if err != nil {
		log.Errorf("Thread Engagement: Could not read IP")
	}

	f := r.Form

	engagement := db.EngagementEvent{
		Wallet:   address,
		JWT:      jwt,
		Activity: 2,
		IP:       ip,
	...
	db.TrackEngagement(engagement)
	return
}

// Create a thread with a closed OP
func createThread(w http.ResponseWriter, r *http.Request) {
	trackThreadEngagement(w, r)
